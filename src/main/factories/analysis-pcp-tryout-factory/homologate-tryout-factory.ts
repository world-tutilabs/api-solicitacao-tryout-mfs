...
export const makeHomologateTryoutController = (): Controller => {
  ...
  const dbHomologationTryoutPCP = new DbHomologationTryoutPCP(
    analysisPCPMysqlRepository,
    analysisPCPMysqlRepository
  );
  const homologationTryoutPCPController = new HomologationTryoutPCPController(
    dbHomologationTryoutPCP
  );
