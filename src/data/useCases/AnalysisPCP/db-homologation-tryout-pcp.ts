...
import {
  IHomologate,
  IHomologationTryoutPCP,
} from "../../../domain/useCases/AnalysisPCP/IHomologation-Tryout-PCP";
...
export class DbHomologationTryoutPCP implements IHomologationTryoutPCP {
  ...
  async homologateTryout({
    id,
    status,
    userHomologate,
    comment,
  }: IHomologate): Promise<void> {
    if (status == 3 || status < 0 || status > 5) {
      throw new AppError("Status indisponível", 403);
    }

    const findHomologate =
      await this.findByHomologateTryoutPCPRepository.findByHomologateTryout(id);
    ...
    const dataHomologate =
      await this.homologationRepositoryInRepository.homologate({
        id,
        status,
        userHomologate,
        comment,
